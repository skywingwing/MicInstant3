...
public class UpdateFeature {
    ...
    //构造函数,为成员变量赋值
    public UpdateFeature(String requestUrl,String APPID,String apiSecret,String apiKey,String AUDIO_PATH){
    ...
    //提供给主函数调用的方法
    public static void doUpdateFeature(String requestUrl,String APPID,String apiSecret,String apiKey,String AUDIO_PATH){
        UpdateFeature updateFeature = new UpdateFeature(requestUrl,APPID,apiSecret,apiKey,AUDIO_PATH);
        try {
            String resp = updateFeature.doRequest();
            System.out.println("resp=>"+resp);
            JsonParse myJsonParse = json.fromJson(resp, JsonParse.class);
            String textBase64Decode=new String(Base64.getDecoder().decode(myJsonParse.payload.updateFeatureRes.text), "UTF-8");
    ...
    class Payload{
        //根据model的取值不同,名字有所变动。
        public UpdateFeatureRes updateFeatureRes;
    }
    class UpdateFeatureRes{
