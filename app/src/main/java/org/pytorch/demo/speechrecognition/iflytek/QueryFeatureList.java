...
public class QueryFeatureList {
    ...
    //构造函数,为成员变量赋值
    public QueryFeatureList(String requestUrl,String APPID,String apiSecret,String apiKey){
    ...
    //提供给主函数调用的方法
    public static void doQueryFeatureList(String requestUrl,String APPID,String apiSecret,String apiKey){
        QueryFeatureList queryFeatureList = new QueryFeatureList(requestUrl,APPID,apiSecret,apiKey);
        try {
            //queryFeatureListRes
            String resp = queryFeatureList.doRequest();
            System.out.println("resp=>"+resp);
            JsonParse myJsonParse = json.fromJson(resp, JsonParse.class);
            String textBase64Decode=new String(Base64.getDecoder().decode(myJsonParse.payload.queryFeatureListRes.text), "UTF-8");
            JSONArray jSONArray = JSON.parseArray(textBase64Decode);
            System.out.println("text字段Base64解码后=>"+jSONArray);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
